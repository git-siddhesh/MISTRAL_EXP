...
# In[]: GPU stats _________________________________________________________________________________________________________
from pynvml import *
...
# In[]: Model size and parameters __________________________________________________________________________________________
    
class HyperParams:
    def __init__(self, epoch = 1, learning_rate = 3e-4, model_id = "mistral"):
        self.learning_rate = learning_rate
        self.epochs = epoch
        self.model_id = model_id
        self.weight_decay=0.1  
        self.warmup_steps=200
        self.lr_scheduler_type="linear" #['linear', 'cosine', 'cosine_with_restarts', 'polynomial', 'constant', 'constant_with_warmup', 'inverse_sqrt', 'reduce_lr_on_plateau']
        self.BATCH_SIZE=1
...
class Dataset_Preprocessing:
    ...
    def tokenize_function(self, examples):
        ...
        return self.tokenizer.encode(examples["text"])
    ...
    def convert_to_hf_dataset(self, df):
        ...
        return dataset.map(self.tokenize_function, batched=True, remove_columns=dataset.column_names, batch_size=1, num_proc=8)
...
# +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
value = [4096,50000,128,14336,4,32,8,8192] # D_emb,Vocal,d_head,d_FF,N_Layer,N_Head,KV_Head,Window
#**************************************************************************************************
...
args = model_obj.mistral_model_args(param)
model = model_obj.get_model(mistral_src=True).to("cuda", dtype=torch.float32)
model_obj.model_size_and_parameters()

# In[]: Dataset __________________________________________________________________________________________________________
dataset_obj = Dataset_Preprocessing(data_path=data_path)
print("Loading tokenizer...")
# tokenizer_path = '/home/dosisiddhesh/MISTRAL_EXP/model/tokenizer_5.0%_50000_new.model'
# tokenizer = dataset_obj.load_tokenizer(tok_type="mistral_src", tokenizer_path=tokenizer_path)

tokenizer_path = "/home/dosisiddhesh/MISTRAL_EXP/model/tokenizer_5.0%_50000_hf.model"
...
tokenizer = dataset_obj.load_tokenizer(tok_type="debertaV2", tokenizer_path=tokenizer_path)

# tokenizer_path = "hf-internal-testing/llama-tokenizer" #llama
# tokenizer = dataset_obj.load_tokenizer(tok_type="llama", tokenizer_path=tokenizer_path)

print("Generating dataset...")
dataset_obj.generate_dataset(rows=1000, eval_frac=0.1)
